...
workflow SEQUENCE_ASSEMBLY {
    ...
    if (params.library_generation_method == 'specific_pcr_umi'){
        ...
        if (params.trim_seq) {
            ch_trim_seq = Channel.from_path(params.trim_seq, checkIfExists: true)
        } else {
            ch_trim_seq = Channel.of([])
        }
        ...
        if (params.trim_seq) {
            error "Please do not set '--trim_seq' when using the 'specific_pcr' library generation method without UMIs."
        }
        ...
        if (params.trim_seq) {
            error "Please do not set '--trim_seq' when using the 'dt_5p_race' library generation method without UMIs."
        }
    } else {
